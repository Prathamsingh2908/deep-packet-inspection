...
- **C++17 compiler**
  - Windows: install MSYS2/MinGW and use `g++`
  - macOS/Linux: use `g++` or `clang++`
...
> Windows note: If `g++` is not recognized, install MSYS2 and add `C:\msys64\ucrt64\bin` to your PATH. Use PowerShell commands with `.\dpi_engine.exe`; the `./dpi_engine` command is for Linux/macOS.
